...
copy.hiwis.ui = function(tosem = first.non.null(app$tosem, app$sem+10),..., app=getApp(), glob=app$glob) {
  restore.point("copy.hiwis.ui")

  ui = modalDialog(size = "l", footer=NULL,
    tagList(
      if (app$stuko & app$admin)
        p("Hinweis: Um als StuKo-Administrator alle Turnusgemaessen hiwis in ein neues Semester zu kopieren, nutzen Sie die entsprechende Option im Reiter 'Admin'"),

      selectInput("tosemCK","hiwis und zugehoerige Module im folgendes Semester kopieren", choices = glob$sets$semester, selected=tosem),
      checkboxInput("tosemOverwriteCK",label="Bereits im neuen Semester angelegte hiwis und Module ueberschreiben"),
      hr(),
      simpleButton("tosemOkCKBtn", "Markierte hiwis und Module kopieren",form.sel = ".hiwiCheck, #tosemCK, #tosemOverwriteCK"), simpleButton("cancelModalBtn","Abbruch")
    )
  )
  ...
  buttonHandler("tosemOkCKBtn",function(formValues,...) {
    ...
    hiwiids = get.selected.hiwiid(formValues)
    cat("Copy hiwiss to", tosem,"...")
    copy.selected.hiwis(tosem=tosem,overwrite = overwrite, hiwiids=hiwiids)
    removeModal()
  })
  ui
}
...
copy.selected.hiwis = function(hiwiids, tosem, overwrite=FALSE, ...,fromsem = getApp()$sem, db=get.stukodb()) {
  restore.point("copy.selected.hiwis")

  sd = get.sem.data(fromsem)

  ku = filter(sd$hiwi, hiwiid %in% hiwiids)

  tosd = get.sem.data(tosem)

  if (!overwrite) {
    ku = filter(ku, ! hiwiid %in% tosd$hiwi$hiwiid)
  }
  if (NROW(ku)==0) return(NULL)

  ku = arrange(ku, desc(aktiv))
  dupl = duplicated(ku$hiwiid)
  ku = ku[!dupl, ]

  kumo = filter(sd$kumo, hiwiid %in% ku$hiwiid)
  kupe = filter(sd$kupe, hiwiid %in% ku$hiwiid)
  ...
  log = paste0("Kopiere ausgew?hlte hiwis und Module von ", semester_name(fromsem), " nach ", semester_name(tosem),"\n\n",
    NROW(ku), " hiwis:\n",
    paste0("  - ", ku$hiwiname, collapse="\n"),
    "\n\n", NROW(mo), " Module:\n",
  ...
  dbWithTransaction(db, {
    if (overwrite) {
      for (hiwiid in ku$hiwiid) {
        dbDelete(db,"hiwi", list(semester=tosem, hiwiid=hiwiid))
        dbDelete(db,"hiwiperson", list(semester=tosem, hiwiid=hiwiid))
        dbDelete(db,"hiwimodul", list(semester=tosem, hiwiid=hiwiid))
      }
      for (hiwiid in ku$hiwiid) {
    ...
    dbInsert(db,"hiwi",ku)
    dbInsert(db,"hiwiperson",kupe)
    dbInsert(db,"hiwimodul",kumo)
